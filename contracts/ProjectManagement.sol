...
contract ProjectManagement {
    ...
    address[] public registeredStudents;  // Todos los estudiantes registrados
    mapping(address => bool) public isStudentAssigned; // Si un estudiante ha sido asignado
    ...
    function registerUser(
        ...
        if (_role == Role.Student) {
            isStudentAssigned[msg.sender] = false;
    ...
    function getUnassignedStudents() public view returns (User[] memory) {
        uint count = 0;
        // Contar estudiantes no asignados
        for (uint i = 0; i < registeredStudents.length; i++) {
            if (!isStudentAssigned[registeredStudents[i]]) {
                count++;
            }
        }

        User[] memory unassignedStudents = new User[](count);
        uint index = 0;
        // Recopilar estudiantes no asignados
        for (uint i = 0; i < registeredStudents.length; i++) {
            if (!isStudentAssigned[registeredStudents[i]]) {
                unassignedStudents[index] = users[registeredStudents[i]];
                index++;
            }
        }
    ...
    function createProject(
        ...
        companyProjects[msg.sender].push(nextProjectId); // Asociar el proyecto con la compañía que lo creó
    ...
    function getCompanyProjects() public view returns (Project[] memory) {
        ...
        uint[] memory projectsIds = companyProjects[msg.sender];
        Project[] memory _projects = new Project[](projectsIds.length);

        for (uint i = 0; i < projectsIds.length; i++) {
            _projects[i] = projects[projectsIds[i]];
        }

        return _projects;
    }

    function getProjectStudents(uint _projectId)
        public
        view
        returns (User[] memory)
    {
        require(
            roles[msg.sender] == Role.Company,
            "Only companies can view project students"
        );

        Project memory project = projects[_projectId];
        User[] memory students = new User[](project.assignedTo.length);

        for (uint i = 0; i < project.assignedTo.length; i++) {
            students[i] = users[project.assignedTo[i]];
        }
    ...
    function getStudentProjects() public view returns (Project[] memory) {
        require(
            roles[msg.sender] == Role.Student,
            "Only student can view their projects"
        );

        uint[] memory projectsIds = studentProjects[msg.sender];
        Project[] memory _projects = new Project[](projectsIds.length);
    ...
    function assignFundsToProject(
        uint _projectId,
        uint _amount
    ) public payable {
        require(
            roles[msg.sender] == Role.Admin,
            "Only admin can assign funds to projects"
        );
        require(_amount > 0, "Amount must be greater than 0");
        require(
            msg.value == _amount,
            "Transaction value must match the amount to assign"
        ); // Asegura que el Ether enviado coincida con el argumento _amount
    ...
    function assignStudentToProject(uint _projectId, address _student) public {
        require(
            roles[msg.sender] == Role.Company,
            "Only companies can assign students"
        );
        require(
            roles[_student] == Role.Student,
            "Can only assign projects to students"
        );
        require(
            projectOwners[_projectId] == msg.sender,
            "Only project owner can assign students"
        );
        require(
            isStudentAssignedToProject(_projectId, _student) == false,
            "Student already assigned to this project"
        );

        Project storage project = projects[_projectId];
        project.assignedTo.push(_student);
        isStudentAssigned[_student] = true;
    ...
    function removeStudentFromProject(uint projectId, address student) public {
        require(
            roles[msg.sender] == Role.Company,
            "Only companies can remove students"
        );
        require(
            projectOwners[projectId] == msg.sender,
            "Only project owner can remove students"
        );

        // Primero, actualizamos la lista del proyecto
        uint studentIndex = findStudentIndex(projectId, student);
        require(
            studentIndex < projects[projectId].assignedTo.length,
            "Student not found in project"
        );

        // Eliminar estudiante del proyecto
        projects[projectId].assignedTo[studentIndex] = projects[projectId]
            .assignedTo[projects[projectId].assignedTo.length - 1];
        ...
        require(
            projectIndex < studentProjects[student].length,
            "Project not found for student"
        );

        // Eliminar proyecto del estudiante
        studentProjects[student][projectIndex] = studentProjects[student][
            studentProjects[student].length - 1
        ];
    ...
    function getProjectsAssignedToStudent(
        address student
    ) public view returns (uint[] memory) {
        return studentProjects[student];
    }

    function isStudentAssignedToProject(
        uint _projectId,
        address _student
    ...
    function viewStudentFunds() public view returns (uint totalFunds) {
        require(
            roles[msg.sender] == Role.Student,
            "Only students can view their funds"
        );
        totalFunds = 0;
    ...
    //     require(
    //         project.funds <= address(this).balance,
    ...
    //     require(
    //         projectOwners[_projectId] == msg.sender,
    //         "Only project owner can change the status"
    //     );
    //     Project storage project = projects[_projectId];
